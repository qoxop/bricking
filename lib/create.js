...
const DefaultTemplate = {
    name: 'my-app-name',
    path: path_1.default.resolve(templatesDir, './default'),
...
const templates = {
    ...
    remote: {
        ...DefaultTemplate,
        path: path_1.default.resolve(templatesDir, './remote')
    }
};
const create = async (tplName = 'default', name = 'my-app') => {
    const template = templates[tplName];
    if (template) {
        (0, fs_tools_1.fileTransfer)(template.path, path_1.default.resolve(process.cwd(), `./${name}`), template.getTransform(name));
    }
    else {
