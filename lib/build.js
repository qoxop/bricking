...
const buildHtml = (options) => {
    ...
    if (!isRemote) {
        sdkEntry = url_1.default.resolve(cdn, sdkEntry);
        systemjs = url_1.default.resolve(cdn, systemjs);
        appEntry = url_1.default.resolve(cdn, appEntry);
    }
    // 引入 system
    ...
    if (realTime) { // 实时 SDK 模块
        // sdkEntry json 
        const combineCode = (0, build_sdk_1.REAL_TIME_SDK)(sdkEntry, url_1.default.resolve(cdn, appEntry));
        const hash = (0, crypto_1.createHash)("sha256").update([combineCode, 't2dkoi1a'].join(":")).digest("hex").slice(0, 8);
        const truesdkEntry = names_1.default.sdkEntry.replace('[hash]', hash);
        fs_1.default.writeFileSync(path_1.default.join(output, truesdkEntry), combineCode);
        const sdkScript = document.createElement('script');
        sdkScript.src = url_1.default.resolve(cdn, truesdkEntry);
    ...
    fs_1.default.writeFileSync(path_1.default.join(output, './index.html'), html_1.dom.serialize(), { encoding: 'utf-8' });
};
...
const build = async (app = false) => {
    ...
    await fs_1.default.promises.writeFile(path_1.default.join(configs.output, `./${names_1.default.moduleInfo}`), JSON.stringify(moduleInfo, null, '\t'));
    // 6. 打 zip 包
    if (pack) {
        await require('zip-dir')(configs.output, {
            saveTo: path_1.default.join(configs.output, moduleInfo.zipPath),
        });
    }
    // 7. 构建完整应用
    if (app) {
