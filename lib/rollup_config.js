...
function rollupConfig(configs, isApp = true) {
    ...
    const assetsOutput = (!isApp && configs.sdk.type === 'local') ? path_1.default.join(configs.sdk.location, configs.assets.relative) : path_1.default.join(configs.output, configs.assets.relative);
    // 输入配置
    const inputConfig = {
