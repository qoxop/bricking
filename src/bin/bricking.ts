...
function buildSdk() {
    process.env.NODE_ENV = 'production';
    require('../scripts/sdk').buildSdk(true);
}

function buildModules(app = false) {
    process.env.NODE_ENV = 'production';
    return require('../scripts/build').build(app);
}

function startDevServe() {
    process.env.NODE_ENV = 'development';
    require('../scripts/dev').start();
}
...
switch(command) {
    case 'create':
        ...
        require('../scripts/create').create(tplName, projName);
        break;
    case 'build':
        if (ARGS['sdk']) {
            buildSdk();
    ...
    case 'serve':
        ...
        ps.then(() => {
            require('../scripts/serve').serve(serveSdk);
        })
        break;
    default:
